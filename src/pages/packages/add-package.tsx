            ...
            <div>
               ...
               <div>
                <select {...register("measure_id")} name="measure_id" id="measure_id" className="block  px-4 py-2 mt-2 text-gray-500 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600">
                    ...
                    { measurelist !== undefined && measurelist.data.data.map( measure => <option key={measure.id}  value={measure.id}>{measure.name}</option>) }
                    </select>            
               </div>
                <div className="w-full mt-4">
                    <label htmlFor="condition" className="text-gray-700 dark:text-gray-200">Conditionnement</label>
                    <input {...register("package")} className="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border rounded-lg dark:bg-gray-800 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring-opacity-40 focus:outline-none focus:ring focus:ring-blue-300" type="number"  />
                </div>
